...
# ════════════════════════════════════════════════════════
# ENDPOINT TEMPORANEO DI MIGRAZIONE (BONIFICA TOTALE)
# ════════════════════════════════════════════════════════
...
async def migrazione_totale(admin: dict = Depends(richiede_admin)):
    ...
    corretti = 0
    for doc_id, nome_file, file_originale in docs:
        if not file_originale: continue
        old_path = Path(file_originale)
        ...
        try:
            with open(old_path, "rb") as f:
                ...
                elif header.startswith(b"II*") or header.startswith(b"MM\x00\x2a") or header.startswith(b"II\x2a\x00"): new_ext = ".tif"
                elif header[:3] == b"\xff\xd8\xff": new_ext = ".jpg"
                elif header.startswith(b"\x89PNG"): new_ext = ".png"
                
                if new_ext:
                    new_nome = Path(nome_file).stem + new_ext
                    new_path = old_path.with_suffix(new_ext)
                    if not new_path.exists():
                        import os
                        os.rename(old_path, new_path)
                    cur.execute(
                        "UPDATE documenti SET nome_file = %s, file_originale = %s WHERE id = %s",
                        (new_nome, str(new_path), doc_id)
                    )
                    corretti += 1
        except Exception: pass
    ...
    cur.close()
    conn.close()
    return {"messaggio": f"Migrazione completata. Corretti {corretti} file."}
