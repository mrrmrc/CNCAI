...
# ════════════════════════════════════════════════════════
# ENDPOINT TEMPORANEO DI BONIFICA CATEGORIE E DOPPIONI
# ════════════════════════════════════════════════════════

@app.post("/admin/bonifica-finale")
async def bonifica_finale():
    conn = get_db()
    cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    
    # 1. Trova le categorie esistenti per beccare "Padre Mario"
    cur.execute("SELECT DISTINCT categoria FROM documenti WHERE categoria IS NOT NULL")
    cats = [r["categoria"] for r in cur.fetchall()]
    
    target_cat = "Documenti Padre Mario" # Default
    for c in cats:
        if "mario" in c.lower():
            target_cat = c
            break
    
    # 2. Sposta gli orfani (Generale/NULL) nella categoria corretta
    cur.execute(
        "UPDATE documenti SET categoria = %s WHERE categoria IS NULL OR categoria = %s OR categoria = %s",
        (target_cat, "", "Generale")
    )
    spostati = cur.rowcount
    
    # 3. Identifica doppioni (stesso nome_file)
    cur.execute("""
        SELECT nome_file, COUNT(*)
        FROM documenti
        GROUP BY nome_file
        HAVING COUNT(*) > 1
    """)
    doppioni = cur.fetchall()
    
    # 4. Eliminazione logica doppioni (tiene il primo ID, cancella gli altri)
    eliminati = 0
    for d in doppioni:
        nome = d["nome_file"]
        cur.execute("SELECT id FROM documenti WHERE nome_file = %s ORDER BY id ASC", (nome,))
        ids = [r["id"] for r in cur.fetchall()]
        # Tiene l'ID più basso (il primo inserito) e cancella gli altri
        to_delete = ids[1:]
        for doc_id in to_delete:
            # Cancella embedding e poi documento
            cur.execute("DELETE FROM embeddings WHERE documento_id = %s", (doc_id,))
            cur.execute("DELETE FROM documenti WHERE id = %s", (doc_id,))
            eliminati += 1
    ...
    conn.close()
    return {
        "target_categoria": target_cat,
        "documenti_spostati": spostati,
        "doppioni_rimossi": eliminati,
        "categorie_trovate": cats
    }
